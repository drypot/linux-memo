...
한번에 쭉 따라할 수 있는 문서가 없어서 이틀간 삽질했다.
...
해서 아래처럼 environment.d 루틴에 환경변수 세팅을 맞겼다.
...
일단 나는 여기까지 하고 재부팅했다.\
uim-xim을 서비스로 등록하기 전에 작동되는지 확인하고 싶었다.

재부팅되면 터미널을 열어서 uim-xim 을 실행시킨다.

    $ uim-xim

다른 환경, 다른 IM에서는 보통 이정도 하면 화면에 언어 선택기가 추가된다.\
...
테스트 과정에서 설정을 잘못해서 uim-xim 이 죽어있었다.\
이 원인으로 IntelliJ에서 한글 입력이 안 되고 있었다.

신기한 것은 일단 gnome 프로그램들에서는 한글입력이 된다는 것이었다.\
무슨 조화인지는 모르겠지만 그랬다.

터미널에서 수동으로 uim-xim 을 띄워보니 IntelliJ에서도 한글이 입력되었다.

## 결론

이렇게 해서 IBus 보다 좀더 그럴듯한 한글 입력 환경을 만들 수 있었다.
